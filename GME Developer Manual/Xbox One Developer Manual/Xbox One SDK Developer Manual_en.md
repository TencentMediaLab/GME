## Overview
Thank you for using Tencent Cloud Game Multimedia Engine SDK. This document provides a detailed description that makes it easy for Xbox One developers to debug and integrate the APIs of Game Multimedia Engine.
...
### Message passing
With the API class, the Delegate method is used to send callback notifications to your application. ITMG_MAIN_EVENT_TYPE indicates the message type. The data under Xbox One is in json string format. See the relevant documentation for the key-value pairs.
...
```
ITMGContext* context = ITMGContextGetInstance();
context->SetLogLevel(TMG_LOG_LEVEL_NONE,TMG_LOG_LEVEL_NONE);
```

### Set the print log path
This API is used to set the print log path.
The default path is:"G:\Tencent\GME".
